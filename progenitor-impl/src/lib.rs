...
impl Generator {
    ...
    pub fn generate_tokens(&mut self, spec: &OpenAPI) -> Result<TokenStream> {
        ...
        // If we have any multipart forms,
        //   we need to generate a multipart form handler
...
fn process_multipart_form(spec: &OpenAPI) -> TokenStream {
    let multipart_forms = spec
        ...
        let mut multipart_form_parts = std::collections::BTreeMap::new();
                                                                ...
                                                                match format {
                                                                    openapiv3::StringFormat::Binary => {
                                                                        multipart_form_parts
                                                                            ...
                                                                            .or_insert_with(BTreeMap::new)
                                                                            .insert("parttype".to_string(),"bytes".to_string());
                                                                    },
                                                                    _ => {
                                                                        multipart_form_parts
                                                                            .entry(operation_id.clone())
                                                                            ...
                                                                            .entry(name.to_string())
                                                                            .or_insert_with(BTreeMap::new)
                                                                            .insert("parttype".to_string(),"text".to_string());
                                                                    },
                                                                }
                                                            },
                                                            _ => {},
    ...
    let into_multipart_list = multipart_form_parts.iter()
        .flat_map(move |(operation_id, property_list)| {
            let parts = property_list.iter().map(move |(partname, properties)| {
                ...
                let partfilename = partname.clone()+".data";
